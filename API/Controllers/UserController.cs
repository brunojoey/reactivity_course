...
namespace API.Controllers
{
    public class UserController : BaseController
    {
        ...
        [AllowAnonymous] // Not expecting a user to authenticate when logging in
        [HttpPost("facebook")]
        ...
        {
            command.RefreshToken = Request.Cookies["refreshToken"]; // pulled from the user
            var user = await Mediator.Send(command);
            SetTokenCookie(user.RefreshToken);
            return user;
        }

        [AllowAnonymous] // user cannot be verified when validating their email
